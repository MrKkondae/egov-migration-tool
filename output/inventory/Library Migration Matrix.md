...
| 유지   | 그대로 사용 가능                 |
| 버전업  | 최신 버전으로 업그레이드             |
| 전환대상   | 다른 라이브러리 또는 프레임워크 기능으로 전환 |
| 제거   | 사용 중단                     |
| 검토   | 별도 분석 필요                  |
| 대상아님 | 전환 대상 아님                  |
...
| --: | ------------------------------------------ | ----------- | -------------- | ---- | -------------------------------- |  ---------------- |
|   1 | antlr-2.7.7.jar                            | Utility     | Parser         | 유지 | antlr-2.7.7.jar                               | Hibernate 종속으로 유지(org.hibernate:hibernate-core:jar:5.6.15.Final) |
|   2 | antlr-3.5.jar                              | Utility     | Parser         | 검토  |                         | 사용처 확인 후 유지/제거 판단 |
|   3 | antlr-runtime-3.5.jar                      | Utility     | Parser Runtime | 검토  |                          | 사용처 확인 후 유지/제거 판단 |
|   4 | aopalliance-1.0.jar                        | Spring      | AOP            | 유지   | aopalliance-1.0.jar                | Spring Dependency
...
|   6 | bcprov-ext-jdk15-1.4.5.jar                 | Security    | 암호화 / **연말정산** | 버전업  | BouncyCastle                     | 자동변환 어려움. org.bouncycastle:bcprov-ext-jdk18on:1.84 Maven 의존성으로 교체하는 것을 권장 |
...
|  10 | commons-dbcp-1.4.jar                       | Persistence | DB Pool        | 전환대상   | Tomcat JDBC, Commons DBCP2, PoolHikariCP (Spring Boot)                         |
...
|  15 | commons-logging-1.1.1.jar                  | Logging     | Logging        | 전환대상   | SLF4J/Logback                    |
|  16 | commons-net-3.3.jar                        | Utility     | FTP 등          | 버전업  | Commons Net                      |
|  17 | commons-pool-1.5.4.jar                     | Persistence | Pool           | 전환대상   | HikariCP                         |
|  18 | commons-validator-1.4.0.jar                | Validation  | 검증             | 검토대상(제거)   | Commons Validator                |
|  19 | egovframework.rte.fdl.cmmn-3.1.0.jar       | eGov        | 공통기능           | 전환대상   | org.egovframe.rte.fdl.cmmn       |
|  20 | egovframework.rte.fdl.idgnr-3.1.0.jar      | eGov        | ID 생성          | 전환대상   | org.egovframe.rte.fdl.idgnr      |
|  21 | egovframework.rte.fdl.logging-3.1.0.jar    | eGov        | 로깅             | 전환대상   | org.egovframe.rte.fdl.logging    |
|  22 | egovframework.rte.fdl.property-3.1.0.jar   | eGov        | Property       | 전환대상   | org.egovframe.rte.fdl.property   |
|  23 | egovframework.rte.psl.dataaccess-3.1.0.jar | eGov        | DAO            | 전환대상   | org.egovframe.rte.psl.dataaccess |
|  24 | egovframework.rte.ptl.mvc-3.1.0.jar        | eGov        | MVC            | 전환대상   | org.egovframe.rte.ptl.mvc        |
|  25 | exportfile-2.0.jar | ETC | 연말정산 | 검토 | 확인 필요 | 국세청 종이없는 연말정산 관련 Lib, 특이사항 없으면 유지 | 
| 26 | `ibatis-sqlmap-2.3.4.726.jar` | Persistence | SQL Mapper | 전환대상 | `mybatis-3.5.x` + `mybatis-spring-2.1.x` / `org.egovframe.rte.psl.dataaccess` | iBatis 사용 범위 확인 후 MyBatis로 통합 전환 필요. `SqlMapClient`, `SqlMapClientTemplate`, `<sqlMap>`, `resultClass`, `parameterClass` 사용 여부 점검 |
| 27 | `icu4j-53.1.jar` | Utility | 국제화 / 문자 처리 | 버전업 | `icu4j` 최신 JDK8 호환 버전 | 유니코드, 로케일, 날짜/문자 처리용. 직접 사용 여부 확인 후 eGov 4.3 환경에서 호환 버전 적용 |
| 28 | `itext-2.1.7.jar` | Document | PDF 생성 / 편집 / **연말정산** | 전환대상 | `OpenPDF 1.3.x` 또는 상용 `iText 7` | iText 5.x(2009년)부터 AGPL로 전환됨 (iText 3.x 공개 배포 버전 미존재). 현재 사용 중인 2.1.7도 IP 불명확성 이슈로 유지 부적합.  OpenPDF(LGPL/MPL, com.lowagie.text 패키지 유지)는 소스 수정 없이 전환 가능하여 1순위 대안. |
...
| 30 | `jackson-core-asl-1.9.13.jar` | JSON | JSON Core | 전환대상 | `com.fasterxml.jackson.core:jackson-core:2.x` | Jackson 1.x 계열에서 2.x 계열로 변경. `org.codehaus.jackson` → `com.fasterxml.jackson` import 변경 필요 |
| 31 | `jackson-mapper-asl-1.9.13.jar` | JSON | JSON Mapping | 전환대상 | `com.fasterxml.jackson.core:jackson-databind:2.x` + `jackson-annotations` | JSON ↔ Java 객체 매핑용. Spring MVC 설정에 `MappingJacksonHttpMessageConverter`가 있으면 `MappingJackson2HttpMessageConverter`로 변경 필요 |
...
| 33 | `jasypt-spring3-1.9.2.jar` | Security / Spring | Spring 암호화 연계 | 전환대상 / 검토 | `jasypt-spring31` 수동 Bean 설정 또는 Spring Boot 전환 시 `jasypt-spring-boot` | Jasypt 공식 1.9.x 라인에는 Spring 5 전용 모듈이 없음. XML 기반 Spring MVC 환경에서는 수동 Bean 설정 방식 검토 |
...
| 36 | `jdbc-14.jar` | Persistence | Oracle JDBC 추정 | 전환대상 / 검토 | `ojdbc8` 단일 적용 | `ojdbc14-10.2.0.1.0.jar`와 중복 가능성 큼. JAR Manifest 확인 후 Oracle JDBC 드라이버는 하나로 정리 필요 |
| 37 | `jdom-1.0.jar` | XML | XML 처리 | 전환대상 / 버전업 | `org.jdom:jdom2:2.x` | JDOM 1.x → JDOM2 전환 시 패키지와 API 변경 가능. XML 생성/파싱 사용 소스 확인 필요 |
| 38 | `jsch-0.1.54.jar` | Network | SSH / SFTP | 전환대상 / 버전업 | `com.github.mwiede:jsch:0.2.x` 또는 대체 라이브러리 | 원본 JCraft JSch는 유지보수가 제한적이므로 mwiede fork 검토. SFTP 업로드/다운로드, 암호화 알고리즘 호환성 테스트 필요 |
...
| 41 | `lib.dstspdfSig-1.1.jar` | Security / Document | PDF 전자서명 추정 / **연말정산** | 검토 | 공급사 최신 JDK8/WAS 호환 버전 | PDF 전자서명 모듈 가능성. 인증서, TSA, PDF 생성 라이브러리와 연계되므로 공급사 확인 및 업무 테스트 필수 |
| 42 | `lib.magic.tsaclient-1.1.jar` | Security | TSA 타임스탬프 클라이언트 추정 / **연말정산** | 검토 | 공급사 최신 JDK8/WAS 호환 버전 | 타임스탬프 서버 연계 모듈 가능성. TSA 서버 주소, 인증서, 네트워크 정책, 공급사 API 변경 여부 확인 필요 |
...
| 50 | `ojdbc14-10.2.0.1.0.jar` | Persistence | Oracle JDBC Driver | 전환대상 | `ojdbc8` 단일 적용 | JDK8 기준 `ojdbc8`로 정리 권장. `jdbc-14.jar`와 중복 제거, DB/WAS 공통 lib 중복 확인, 드라이버 클래스 `oracle.jdbc.OracleDriver` 적용 확인 |
...
|  64 | spring-modules-validation-0.9.jar         | Validation | 검증                 | 전환대상   | Bean Validation / Hibernate Validator | Spring Modules Validation 제거, Bean Validation 기반 구조 적용, Hibernate Validator 적용, Annotation 기반 Validation 적용, Validation 메시지 처리 구조 개선
...
|  70 | standard-1.1.2.jar                        | Web        | JSTL 구현            | 전환대상   | taglibs-standard-impl-1.2.5       | standard.jar 제거, JSTL 1.2 적용, 소스 수정 없음
